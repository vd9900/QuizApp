...
const TypeOfQuestions = ["Pipes and Cisterns", "Probability", "Problems on age", "Profit and loss"]//To get the index number which user selected
const QuizArray = [
    [
...
function openClose(open, close, button) {
    button.addEventListener("click", function () {
        if (!UserInput.value) {
            alert("Nam kya betuwa?")
        } else {
            if (close == Section1) {//When the section1 container will close then 3s timer will set in I agrre btn
                SetTimer(3, btn3)
            }
            if (close == Section2) {
                MainQuiz();
...
function selecting(array) {
    array.forEach(element => {//Targeting the all element of options to run event click
...
function QuizAnswerSelect(options) {
    ...
    options.addEventListener("click", function (e) {
        if (e.target.tagName == "SPAN") {//e.target is shows that where user clicked
            if (e.target.classList.contains("selected")) {
                ...
                UserAnwser = Array.from(option)/*option is collection of all option it's type is nodelist so,
                converting into array then easy can access the indexof the each option */
               UserAnwseredIndex = UserAnwser.indexOf(e.target)
            }
            (e.target.classList.contains("selected")) ? userAnwserIndex = UserAnwseredIndex : userAnwserIndex = undefined;
...
function nextGo(parent) {
    ...
    parent.addEventListener("click", function (e) { //Targeting the parent to target the chlid element
        if (e.target.tagName == "SPAN" || e.target.tagName == "I") {  // event delegation
            if (e.target.id == "next" || e.target.classList.contains("fa-arrow-right")) {
...
function MainQuiz() { //It will change the questions
    UserSelectedArray = QuizArray[TypeOfQuestions.indexOf(SeletedOption)] // Array of selected quiz Categorey 
    NumberQuestion = UserSelectedArray[QuestionNumber];/* QuestionNumber start with 0 and when function NextGo or timerforquestion is called
...
function ReslutPage() {
    playerName.textContent = `Player : ${UserInput.value}`;
    totaltimetaken.textContent = (timetaken > 9) ? `Total time taken : 0:${timetaken}s` : `Total time taken : 0:0${timetaken}s`;
    totalque.textContent = `Total Questions : ${QuestionNumber + 1}`;
    attempt.textContent = `Attempted : ${totalattempt}`;
    correct.textContent = `Correct : ${correctAnwesers}`
    worng.textContent = `Worng : ${wornganwser}`;
    percentage.textContent = `Percentage : ${((correctAnwesers * 100) / 10)}%`;
...
function CheckTheAnswser() {
    if (userAnwserIndex == NumberQuestion.anwser) {//checking user anwser if it's correct Anwesers   
        // console.log(Adbuth)
        totalattempt++;
        correctAnwesers++;
